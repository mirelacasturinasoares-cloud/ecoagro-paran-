...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ==========================================
    // MENU HAMBÚRGUER (3 PONTINHOS / LINHAS)
    // ==========================================
    // Garanta que no seu HTML o botão tenha a classe 'menu-btn' ou 'hamburger' 
    // e os links do menu tenham a classe 'nav-links'
    const menuBtn = document.querySelector('.menu-btn') || document.querySelector('.hamburger');
    const navLinks = document.querySelector('.nav-links');

    if (menuBtn && navLinks) {
        // Suporta clique no PC e toque no celular de forma instantânea
        const alternarMenu = (e) => {
            e.preventDefault();
            navLinks.classList.toggle('active');
        };
        menuBtn.addEventListener('click', alternarMenu);
        menuBtn.addEventListener('touchstart', alternarMenu, { passive: false });
    }
    ...
    window.mostrarSustentavel = function () {
        const res = document.getElementById("resultadoComparador");
        if (res) {
            res.innerHTML = `
    ...
    // ===================================================
    // CANVAS GRÁFICO TOTALMENTE INTERATIVO (MOUSE & TOUCH)
    // ===================================================
    ...
    if (canvas) {
        ...
        let barWidth = 50; // Diminuído um pouco para caber melhor em telas menores
        let spacing = 35;
        let startX = 30;
        ...
        function desenharGrafico() {
            ...
            for(let l = 1; l <= 4; l++) {
                ...
                ctx.beginPath();
                ctx.moveTo(10, yLine);
                ctx.lineTo(canvas.width - 10, yLine);
                ctx.stroke();
            }
            ...
            valores.forEach((val, idx) => {
                ...
                ctx.fillText(anos[idx], x + 8, canvasHeight - 20);
            });
        }
        ...
        canvas.addEventListener("mouseleave", () => { hoveredIndex = -1; desenharGrafico(); });
        canvas.addEventListener("touchend", () => { hoveredIndex = -1; desenharGrafico(); });

        desenharGrafico();
    }

    // ANIMAÇÃO DOS ELEMENTOS VIA SCROLL (Suavizada para mobile)
    const elementos = document.querySelectorAll("section, .card, .evento");
    function revelar() {
        const tela = window.innerHeight * 0.95; // Aumentado para carregar antes no mobile
    ...
    elementos.forEach(item => {
        item.style.opacity = "1"; // Deixado visível por padrão para evitar travar escondido no mobile
        item.style.transform = "translateY(0)";
    });
    
    // Se o dispositivo for PC, aplica o efeito de surgir, se for mobile carrega direto
...
function calcularFazenda() {
    const energia = parseInt(document.getElementById("energia").value) || 0;
    const irrigacao = parseInt(document.getElementById("irrigacao").value) || 0;
    const monitoramento = parseInt(document.getElementById("monitoramento").value) || 0;
    const preservacao = parseInt(document.getElementById("preservacao").value) || 0;

    const points = energia + irrigacao + monitoramento + preservacao;
    window.simuladorFeito = true;
    let resultado = "";

    if (points >= 85) {
        resultado = `<span style='color:#2e7d32;'>🏆 Nota: ${points}% — Fazenda Modelo!</span><br><small style='font-size:1rem; font-weight:400; color:#555;'>Parabéns! Sua propriedade atinge os maiores critérios técnicos de conservação e produtividade digital.</small>`;
    } else if (points >= 60) {
        resultado = `<span style='color:#ffa000;'>🌱 Nota: ${points}% — Fazenda Sustentável</span><br><small style='font-size:1rem; font-weight:400; color:#555;'>Bom trabalho. Sua fazenda adota boas diretrizes ecológicas, mas ainda pode investir mais em inteligência automatizada.</small>`;
    } else {
        resultado = `<span style='color:#f44336;'>⚠ Nota: ${points}% — Em Adaptação</span><br><small style='font-size:1rem; font-weight:400; color:#555;'>A infraestrutura atual está defasada. Considere aplicar fontes fotovoltaicas ou captação por sensores para progredir.</small>`;
    }
    const resDiv = document.getElementById("resultadoSimulador");
    if(resDiv) resDiv.innerHTML = resultado;
}
